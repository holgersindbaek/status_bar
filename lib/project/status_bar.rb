...
class StatusBar
  ...
  def show_status_bar
    if !statusbar_view_visible?
      ...
      0.3.second.later { @statusbar_view.move_to([statusbar_view_frame(Device.interface_orientation).origin.x, statusbar_view_frame(Device.interface_orientation).origin.y]) }
    end
  end
  ...
  def hide_statusbar_view
    if statusbar_view_visible?
      ...
      0.3.second.later { App.shared.setStatusBarHidden(false, withAnimation:UIStatusBarAnimationSlide) }
    end
  end
